            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    ...
                    <a href="<?= $actionPage ?>" style="float: right; margin-top: 0;"
                        class="btn btn-raised btn-default btn-rounded">Add Area</a>
                </div>
                <div class="panel-body">
                    <div class="basic-datatable-block">
                        <table id="basic_datatable" class="display table table-bordered basic-data-table">
                            ...
                            <tbody>
                                <?php
                                $viewQuery = $conn->query("SELECT $tableName.*, state.state, city.city FROM $tableName JOIN `state` ON $tableName.stateid = state.id JOIN `city` ON $tableName.cityid = city.id ORDER BY `city` ASC");
                                while ($viewRow = $viewQuery->fetch_assoc()) {
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo $viewRow['suburb'] ?></td>
                                        <td><?php echo $viewRow['postcode'] ?></td>
