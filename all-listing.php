...
<div class="page-container-wrap">
    <div class="container-fluid">
        <div class="row">
            <?php
            $activeListingQuery = $conn->query("SELECT * FROM `listings` WHERE `email` = '$user_email' ORDER BY `listtype` DESC");
            if ($activeListingQuery->num_rows == 0) {
                ?>
                <h2>No listing is added yet</h2>
                <?php
            } else {
...
<div class="listing-modal-content-section">
    <?php
    $activeListingQueryModal = $conn->query("SELECT * FROM `listings` WHERE `email` = '$user_email' ORDER BY `listtype` DESC");
