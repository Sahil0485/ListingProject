...
$pages = $_GET['pages'];
$link = $_GET['link'];
$pageQuery = $conn->query("SELECT * FROM `listings` WHERE `pages` = '$pages' AND `link` = '$link'");
$pageRow = $pageQuery->fetch_assoc();
?>
<div class="subheader">
    <h2><?php echo $pageRow['name'] ?></h2>
...
<div class="breadcrumb-block">
    <div class="container">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="sub-category.php?pages=<?php echo $pageRow['link']?>"><?php echo $pageRow['fname'] ?> </a></li>
            <li class="breadcrumb-item active"><?php echo $pageRow['name'] ?></li>
        </ol>
    </div>
</div>
</div>
<div class="popular-listing-section">
    <div class="container">
        <div class="row">
            <?php
            $listingQuery = $conn->query("SELECT * FROM `listings` WHERE `status` = 'Active' AND `pages` = '$pages' AND `link` = '$link'");
            if ($listingQuery->num_rows == 0) {
                echo "<script>";
                echo "alert('This page is empty');";
                echo "window.location.href='all-category.php';";
                echo "</script>";
                exit();
            }
                        ...
                        <div class="post-details">
                            <div class="post-meta">
                                <div class="location">
                                    ...
                                    <h5>
                                        <a href="state-listing.php?statelink=<?php echo $listingRow['statelink'] ?>">
                                            <?php echo $listingRow['state'] ?>
                                        </a>
                                    </h5>
                                </div>
                                <div class="tag">
...
<div class="listing-modal-content-section">
    <div class="container">
        <?php
        $listingQueryModal = $conn->query("SELECT * FROM `listings` WHERE `pages` = '$pages' AND `link` = '$link' AND `status` = 'Active' ORDER BY `listtype` DESC");
                                        ...
                                        <div class="post-details">
                                            <div class="post-meta">
                                                <div class="location">
                                                    ...
                                                    <h5>
                                                        <a
                                                            href="state-listing.php?statelink=<?php echo $listingRow['statelink'] ?>">
                                                            <?php echo $listingRow['state'] ?>
                                                        </a>
                                                    </h5>
                                                </div>
