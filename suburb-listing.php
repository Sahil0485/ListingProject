...
<?php
$suburblink = $_GET['suburblink'];
$pageQuery = $conn->query("SELECT * FROM `listings` WHERE `suburblink` = '$suburblink'");
$pageRow = $pageQuery->fetch_assoc();
?>
<div class="subheader">
    <h2><?php echo $pageRow['suburb'] ?></h2>
...
<div class="breadcrumb-block">
    <div class="container">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a
                    href="city-listing.php?citylink=<?php echo $pageRow['citylink'] ?>"><?php echo $pageRow['city'] ?>
                </a></li>
            <li class="breadcrumb-item active"><?php echo $pageRow['suburb'] ?></li>
        </ol>
    </div>
</div>
</div>
<div class="popular-listing-section">
    <div class="container">
        <div class="row">
            <?php
            $listingQuery = $conn->query("SELECT * FROM `listings` WHERE `status` = 'Active' AND `suburblink` = '$suburblink'");
            if ($listingQuery->num_rows == 0) {
                echo "<script>";
                echo "alert('This page is empty');";
                echo "window.location.href='city-listing.php?citylink=" . $pageRow['citylink'] . ";";
                echo "</script>";
                exit();
            }
...
<div class="listing-modal-content-section">
    <div class="container">
        <?php
        $listingQueryModal = $conn->query("SELECT * FROM `listings` WHERE `suburblink` = '$suburblink' AND `status` = 'Active' ORDER BY `listtype` DESC");
